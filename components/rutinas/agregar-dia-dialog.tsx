...
import { addRoutineDay } from "@/app/(dashboard)/dashboard/alumnos/[id]/rutinas/actions";
...
export function AgregarDiaDialog({
  routineId,
  memberId,
  onDayAdded,
}: {
  ...
  async function handleSubmit(values: RoutineDayNameValues) {
    setLoading(true);
    const result = await addRoutineDay(routineId, memberId, values.name);
    setLoading(false);
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Sumá otro día de entrenamiento a esta rutina.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
