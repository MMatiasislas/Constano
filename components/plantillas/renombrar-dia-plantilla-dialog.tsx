...
import { renameTemplateDay } from "@/app/(dashboard)/dashboard/rutinas/plantillas/[templateId]/actions";
...
export function RenombrarDiaPlantillaDialog({
  day,
  templateId,
}: {
  ...
  async function handleSubmit(values: TemplateDayNameValues) {
    setLoading(true);
    const result = await renameTemplateDay(day.id, templateId, values.name);
    setLoading(false);
